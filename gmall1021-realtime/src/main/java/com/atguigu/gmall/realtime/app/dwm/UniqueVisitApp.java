...
//        System.setProperty("HADOOP_USER_NAME","atguigu");
        ...
        // 对读取的数据进行结构的转换
        SingleOutputStreamOperator<JSONObject> jsonObjStream = jsonStream.map(JSON::parseObject);
        jsonObjStream.print("uv:");

        // TODO: 2021/4/20 核心的过滤代码
        // 按照id分组
        KeyedStream<JSONObject, String> keyedStream = jsonObjStream.keyBy(jsonObj -> jsonObj.getJSONObject("common").getString("mid"));

        SingleOutputStreamOperator<JSONObject> filterJsonObjDstream = keyedStream.filter(new RichFilterFunction<JSONObject>() {
            ...
            public void open(Configuration parameters) throws Exception {
                ...
                if (lastVisitDateState == null) {
                    ValueStateDescriptor<String> lastViewDateStateDescriptor = new ValueStateDescriptor<>("lastVisitDateState", String.class);
                    // 因为统计的是当日uv，也就是日活，所有为状态设置失效时间
                    StateTtlConfig stateTtlConfig = StateTtlConfig.newBuilder(Time.days(1))
                    // 默认值 表名当状态创建或每次写入时都会更新状态戳
            ...
            // 首页检查当前页面是否有上页标识，如果有说明该次访问一定不是当日首次
            @Override
            public boolean filter(JSONObject jsonObject) throws Exception {
                ...
                if (lastViewDate != null && lastViewDate.length() > 0 && logDate.equals(lastViewDate)) {
                    ...
                    System.out.println("未访问：lastVisit:" + lastViewDate + "|| lagDate：" + logDate);
        ...
        SingleOutputStreamOperator<String> dataJsonStringDstream = filterJsonObjDstream.map(JSONAware::toJSONString);
